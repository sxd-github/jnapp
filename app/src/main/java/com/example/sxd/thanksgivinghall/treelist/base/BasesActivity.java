...
public class BasesActivity extends BaseActivity implements BasesContract.View{
    ...
    @Override
    public void getUserInfos(OfficeUserEntity value) {

        for(int i=0;i<value.getData().size();i++) {
            mDatas.add(new Node(value.getData().get(i).getId(), value.getData().get(i).getpId(),value.getData().get(i).getUserId(),
                    value.getData().get(i).getName(),value.getData().get(i).getType()));
        }
    ...
    public void click(View v){
        ...
        for (int i = 0; i < allNodes.size(); i++) {
            if (allNodes.get(i).isChecked()){
                if(allNodes.get(i).getType().equals("人员")) {
